...
//陀螺仪寄存器结构体定义
struct gyro_reg_s {
    unsigned char who_am_i;//---0x75 117 RO -[6:1]- 
    unsigned char rate_div;//---0x19 25 RW [7:0]
    unsigned char lpf;//---0x1A
    unsigned char prod_id;//---0x0C
    unsigned char user_ctrl;//---0x6A 106 - FIFO_EN I2C_MST_EN I2C_IF_DIS - FIFO_RESET I2C_MST_RESET SIG_COND_RESET
    unsigned char fifo_en;//0x23 35 
    unsigned char gyro_cfg;//0x1B 27 XG_ST YG_ST ZG_ST FS_SEL[4:3]
    unsigned char accel_cfg;//0x1C 28 XA_ST YA_ST ZA_ST AFS_SEL[1:0]
    ...
    unsigned char fifo_count_h;//0x72 114 RO FIFO_COUNT[15:8]
    unsigned char fifo_r_w;//---0x74 116 RW FIFO_DATA[7:0]
    unsigned char raw_gyro;//---0x43 0x43-0x48 gyro-x-y-z
    unsigned char raw_accel;//---0x3B 0x3B-0x40 accel-x-y-z 
    unsigned char temp;//---0x41 0x41-0x42
    unsigned char int_enable;//---0x38 56 RW interrupt enable
    unsigned char dmp_int_status;//---0x39 57
    unsigned char int_status;//---0x3A 58 RO 

    //unsigned char accel_intel;

    unsigned char pwr_mgmt_1;//---0x6B 107
    unsigned char pwr_mgmt_2;//---0x6C 108 
    unsigned char int_pin_cfg;//---0x37 55 RW
    unsigned char mem_r_w;//---0x6F 
    unsigned char accel_offs;//---0x06
    unsigned char i2c_mst;//---0x24  36 I2C_MST_CTRL
...
struct chip_cfg_s {
    ...
    unsigned char gyro_fsr;//FS_SEL
    /* Matches accel_cfg >> 3 & 0x03 */
    unsigned char accel_fsr;//AFS_SEL
