...
static void NRF24L01_Analyse(void)
{
	...
	if(NRF24L01_RXDATA[0]=='+'||NRF24L01_RXDATA[0]=='-')
	{
		
	    bat = NRF24L01_RXDATA[11]/100.0;
//		printf("bat1:%f",bat);
		bat += NRF24L01_RXDATA[9];
//		printf("bat2:%f",bat);
		bat -= 11.5;
//		printf("bat3:%f",bat);
		processbar = (u8)(bat*100);
	}
}
...
//		NRF_Send_TX((u8*)&tx_frame,32);
	...
	{	
		NRF24L01_HandlerIRQ();    //清除中断标志位
	}
	if(Nrf_Erro>=500)   //1s未接收nrf数据 
	{	
//		LED2_OFF;
		Nrf_Erro = 1;
//		Rc_Lock=1;  //上锁
